...
for(a in 1:nrow(acc)){
  ...
  if(length(df) == 90){
    df$Name = as.character(acc$Name[a])
    df$MP_id = acc$MP_id[a]
...
write.csv(summary_df, "Politics/download_summary.csv")
saveRDS(comb_df, "Politics/download.rds")
